...
class StoryMenuState extends MusicBeatState // I suck at coding so this will suffice for now
{
	...
	override function create()
	{
		...
		storyMenuCamTween = FlxTween.tween(FlxG.camera, {zoom: 1.4}, 1.1, {type: PINGPONG});

		var bg:FlxSprite = new FlxSprite(-1500,- 714).loadGraphic(Paths.image('menuPatternDesat'));
		bg.color = 0xFF141414;
		bg.antialiasing = ClientPrefs.globalAntialiasing;
		add(bg);

		FlxTween.tween(bg, {x: -924, y: -392}, 12, {type: LOOPING});

		var chooseDiffText:FlxText = new FlxText(0, 260, 0, 'CHOOSE A DIFFICULTY');
		chooseDiffText.setFormat("VCR OSD Mono", 32);
		chooseDiffText.screenCenter(X);

		scoreText = new FlxText(0, 440);
		...
		leftArrow = new FlxSprite(433, 320);
	...
	override function update(elapsed:Float)
	{
		...
		if (!movedBack && !selectedWeek && !autoMode)
		{
			...
			{
				FlxG.camera.zoom = 1;
				storyMenuCamTween.active = false;
