...
class NoteOffsetState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		add(platform_right);
		FlxTween.tween(platform_left, {y: -300}, 1.6, {ease: FlxEase.sineInOut, type: PINGPONG});
		FlxTween.tween(platform_right, {y: 202}, 1.2, {ease: FlxEase.sineInOut, type: PINGPONG});

		var ground:BGSprite = new BGSprite('cheez/ground', -860, 572);
		add(ground);

		// Decided to not load this in since you wont be able to see it due to how zoomed the camera is
		/*var roof:BGSprite = new BGSprite('cheez/roof', -860, -618, 1.15, 0.8);
		add(roof);*/

		// Characters
		gf = new Character(379, 137, 'gf-new');
		gf.x += gf.positionArray[0];
		gf.y += gf.positionArray[1];
		gf.scrollFactor.set(0.95, 0.95);
		boyfriend = new Character(840, 100, 'bf-new', true);
